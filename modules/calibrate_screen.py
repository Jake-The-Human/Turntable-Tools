...
class CalibrateScreen:
    def __init__(self, calibrate_mode: CalibrateMode) -> None:
        ...
        self._text_accelerometer_offset = label.Label(
            FONT,
            text=f"{accel_x:.2f} {accel_y:.2f} {accel_z:.2f}",
            color=DisplayColor.WHITE,
            y=20,
        )
        ...
        self._text_gyro_offset = label.Label(
            FONT,
            text=f"{gyro_x:.2f} {gyro_y:.2f} {gyro_z:.2f}",
            color=DisplayColor.WHITE,
            y=50,
        )
    ...
    def update(self, calibrate_mode: CalibrateMode) -> None:
        """Update the rumble number on screen"""
        if calibrate_mode.is_recording_data():
            self._text_progress.text = STRINGS.CALIBRATE
